...
import { useAllEventsByStatus } from "services/hooks";
  ...
  const panes = [
    {
      id: 1,
      menuItem: "Pending",
      render: () => <Tab.Pane>{getBlockChallenge("Pending", events)}</Tab.Pane>,
    },
    {
      id: 2,
      menuItem: "Wins",
      render: () => <Tab.Pane>{getBlockChallenge("Wins", events)}</Tab.Pane>,
    },
    {
      id: 3,
      menuItem: "Expired",
      render: () => <Tab.Pane>{getBlockChallenge("Expired", events)}</Tab.Pane>,
    },
    {
      id: 4,
      menuItem: "All",
  ...
  return (
    <>
      ...
      <Tab
        ...
        onTabChange={(e, data) => {
          prop.onUpdateItem("keyMyMatch", data.activeIndex);
        }}
      />
    </>
  );
