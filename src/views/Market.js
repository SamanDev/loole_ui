...
import { printProductBlock} from "components/include";
...
const getBlockChallenge = (filtermode,products) => {
  var newItem = []
     ...
     products.map((item, i) => {
     if (item.title.toLowerCase().indexOf(filtermode.toLowerCase()) > -1 || filtermode == 'All'){
       newItem.push(item)
    ...
    return (<Card.Group className="fours" style={{ marginBottom: 20 }}>{printBlockProduct(newItem)}</Card.Group>)
...
var currentUser = prop.findStateId(myState,'currentUser');
  ...
  <Spinner animation="grow" size="sm" /></h4>;
    ...
    const panes = [
      ...
      {id:2, menuItem: (
        <Menu.Item key={"2"}>
          PSN <Label color='teal' size='mini' >{findStateId(myStateThis, "PSN")}</Label>
        </Menu.Item>
      ), render: () => <Tab.Pane>{getBlockChallenge('PSN',products)}</Tab.Pane> },
      {id:3, menuItem: (
        <Menu.Item key={"3"}>
          XBox<Label color='orange' size='mini' >{findStateId(myStateThis, "XBox")}</Label>
        </Menu.Item>
      ), render: () => <Tab.Pane>{getBlockChallenge('XBox',products)}</Tab.Pane> },
      {id:4, menuItem: (
        <Menu.Item key={"4"}>
          Steam <Label color='black' size='mini' >{findStateId(myStateThis, "Steam")}</Label>
        </Menu.Item>
